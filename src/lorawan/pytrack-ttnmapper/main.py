...
# connection.start(force_join = False, str_eui = '70B3D57ED000AF3E', str_key = '2F7022AA819A2469653F1B6DE7889C13')
...
if( APP_EUI == 'INSERTAPPEUI' or APP_KEY == 'INSERTAPPKEY'):
    ...
    print("Register the following device EUI with the application and disable Frame counter check in the settings.")
    print(binascii.hexlify(lora.mac()).upper().decode('utf-8'))
    print("Now enter the INSERTAPPEUI and INSERTAPPKEY above")
else:
    connection.start(force_join = False, str_eui = APP_EUI, str_key = APP_KEY)
...
ledcolor = 'red'                            # first start with color red (no gps position)

while True:
    # -----------------------------
    gc.collect()

    wdt.feed()                                              # Feed the WDT to prevent it from resetting the system
    ...
    if (coord[0] == None):
        ledcolor = 'red'                        # color red (no gps position)
    else:
    ...
    # msgSd = str(count_tx) + ',' + lora_mac + ',' + streset + ',' + cause + ',' + str(coord)
    # ok
    # msgSd = str(count_tx) + ',' + lora_id + ',' + str(coord)
    # note:
    # 5 char of index, for 5 seconds, the max index value is 99999. It restart after 5 days
    # msgSd = "{: 5d},{},({: >8},{: >8})".format(
    msgSd = "{:5d},{},({: >8},{: >8})".format(
            count_tx,
            lora_id,
    ...
    #----------------------------------------------
    # write on file
    # see: https://stackoverflow.com/questions/5214578/python-print-string-to-text-file
