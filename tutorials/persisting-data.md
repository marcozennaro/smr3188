...
The source code is in the `src/flash` directory.

## Folder structure
The folder tree of a Pycom device is the following:
...
main.py under `src/flash/flash` directory:
```python
import os
file_path = '/flash/log'
...
2. if the folder does not exist an error occurs (it raises an OSError **exception**) that we intercept to write our message and create our folder.
